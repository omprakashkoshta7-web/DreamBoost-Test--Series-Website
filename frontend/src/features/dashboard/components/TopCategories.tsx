  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => navigate('/app/exam-categories')} className="text-sm text-tb-blue font-medium flex items-center gap-1 hover:text-tb-blue-dark transition-colors">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {categories.slice(0, 8).map((cat: any, i: number) => (
          <div
            key={cat._id || i}
            onClick={() => navigate(`/app/exam-categories/${cat.slug || cat._id}`)}
              ...
              <div className="flex flex-col items-center text-center gap-3">
                {cat.image ? (
                  ...
                  <div className="w-12 h-12 bg-tb-blue-light rounded-xl flex items-center justify-center group-hover:bg-tb-blue group-hover:text-white transition-colors">
                    <BookOpen className="w-6 h-6 text-tb-blue group-hover:text-white transition-colors" />
                  </div>
                )}
                <div>
